...
def setVariablesBack(counter1, pCO2, temp, mbar, DLI, summePCO2, summeTemp, summembar, summeDLI, avPCO2, avTemp, avmbar, avDLI, csvRow, loopedTime, csvTimeCounter, date, x, plotAvPCO2, plotAvTemp):
    counter1 = 0
    pCO2 = 0
    temp = 0
    mbar = 0
...
while counter1 < 8:
    if counter2 <= 1440:
        ...
        if client.connect():  # Trying to connect to Modbus Server/Slave
            ...
            if not res.isError():  # If Registers don't show Error
                ...
                if counter1 == 6:
                    ...
                    writeInCSVFile(summePCO2, summeTemp, summembar, summeDLI, startTime, adresse, csvRow)

                    plotFromCSV(a, b, adresse, x, plotAvPCO2, plotAvTemp, date)

                    print('----------------------------------------------------------------')
                    print('')

                    setVariablesBack(counter1, pCO2, temp, mbar, DLI, summePCO2, summeTemp, summembar, summeDLI, avPCO2, avTemp, avmbar, avDLI, csvRow, loopedTime, csvTimeCounter, date, x,plotAvPCO2, plotAvTemp):
        ...
        startNewTimedLoop(a, b, realTemp, counter2, adresse)